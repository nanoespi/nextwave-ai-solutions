...
- [X] `npm run dev` runs and all sections render correctly
- [X] Particle system cycles through at least 3 patterns without errors in console
- [X] Chat widget opens, closes, and sends/receives messages
- [X] All four button variants render correctly across sections
- [X] Mobile breakpoint (375px) is fully usable — no horizontal scroll, no clipped text
- [X] All `data-reveal` animations fire on scroll, skip correctly under `prefers-reduced-motion`
- [X] No purple, electric blue, or gradient colors appear anywhere on the page
- [X] Voltage appears exactly three times as a primary element (hero headline, hero CTA, final CTA button)
- [X] Brand JSON is present at project root and referenced in `README.md` as the design system source of truth
